...
tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=True, trust_remote_code=True, cache_dir="/scratch/user/u.kw178339/huggingface_models")
tokenizer.pad_token = tokenizer.eos_token
model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.bfloat16, trust_remote_code=True, cache_dir="/scratch/user/u.kw178339/huggingface_models")
...
if model_name == "mistralai/Mistral-7B-Instruct-v0.3":
    dataset_path = "/scratch/user/u.kw178339/GenderBias/StoryGeneration/swapped_explanations_mistral.jsonl"
else:
    dataset_path = "/scratch/user/u.kw178339/GenderBias/StoryGeneration/swapped_explanations.jsonl"
print(dataset_path)
...
training_args = TrainingArguments(
    output_dir=f"./{model_short_name}-cda-lora-finetuned-{dataset_params}",
    per_device_train_batch_size=2,
    gradient_accumulation_steps=8,
    num_train_epochs=1,
    learning_rate=2e-4,
    warmup_steps=100,
    logging_dir=f"./logs/{model_short_name}-{dataset_params}",
    logging_steps=100,
    save_steps=500,
    save_total_limit=2,
    bf16=True,
...
# === Step 11: Save ===
final_model_dir = f"./{model_short_name}-cda-finetuned-{dataset_params}"
